...
export class TicketsService {
  ...
  async update(id: number, data: any) {
    try {
      ...
      const result = await db.update(tickets).set(data).where(eq(tickets.id, id)).returning()
    ...
    try {
      ...
      const messagesWithSender = await Promise.all(
        messages.map(async (message) => {
          // Get user info
          const userResult = await db.select().from(users).where(eq(users.id, message.userId)).limit(1)
          ...
          const attachments = await db
            ...
            .where(and(eq(ticketAttachments.ticketId, ticketId), eq(ticketAttachments.userId, message.userId)))
  ...
  async addAttachments(attachments: any[]) {
    try {
      ...
      const result = await db.insert(ticketAttachments).values(attachments).returning()
      return result
    } catch (error) {
  ...
  async getAttachment(attachmentId: number) {
    try {
      const result = await db.select().from(ticketAttachments).where(eq(ticketAttachments.id, attachmentId)).limit(1)
  ...
  async deleteAttachment(attachmentId: number) {
    try {
      const result = await db.delete(ticketAttachments).where(eq(ticketAttachments.id, attachmentId)).returning()
  ...
  async findByAssignee(assigneeId: number) {
    try {
      return await db.select().from(tickets).where(eq(tickets.assignedTo, assigneeId)).orderBy(desc(tickets.createdAt))
    } catch (error) {
  ...
  async getTicketStats(userId?: number) {
    try {
      ...
      const totalTickets = await db.select({ count: sql<number>`count(*)::int` }).from(tickets).where(conditions)
  ...
  async findByDepartment(department: string) {
    try {
      return await db.select().from(tickets).where(eq(tickets.department, department)).orderBy(desc(tickets.createdAt))
    } catch (error) {
  ...
  async getTicketList(filters: {
    ...
    try {
      ...
      if (filters.assignedTo) {
        conditions.push(eq(tickets.assignedTo, Number.parseInt(filters.assignedTo)))
      }

      if (filters.userId) {
        conditions.push(eq(tickets.userId, Number.parseInt(filters.userId)))
      }

      if (filters.assignedOrCreated) {
        const id = Number.parseInt(filters.assignedOrCreated)
        conditions.push(or(eq(tickets.userId, id), eq(tickets.assignedTo, id), eq(tickets.currentHandler, id)))
      }
  ...
  async getExecutiveDashboard(filters?: {
    ...
    try {
      ...
      const userPerformance = await db
        ...
        .where(and(ne(tickets.assignedTo, null), ...(conditions.length > 0 ? conditions : [])))
  ...
  private async getTicketTrendsOverTime(filters?: {
    ...
    try {
      ...
      const conditions = [gte(tickets.createdAt, startDate), lte(tickets.createdAt, endDate)]
      ...
      const newTicketsData = await db
        .select({
          date: sql<Date>`date_trunc('day', created_at)::date`,
          count: sql<number>`count(*)::int`,
        })
        .from(tickets)
        .where(and(...conditions))
      ...
      const resolvedTicketsData = await db
        .select({
          date: sql<Date>`date_trunc('day', completed_at)::date`,
          count: sql<number>`count(*)::int`,
        })
        .from(tickets)
        .where(
  ...
  async updateSLAStatus() {
    try {
      ...
      for (const ticket of breachedTickets) {
        // Update status
        await db.update(tickets).set({ slaStatus: "breached" }).where(eq(tickets.id, ticket.id))
      ...
      const userLoads = await Promise.all(
        users.map(async (user) => {
          const activeTickets = await db
            ...
            .from(tickets)
            .where(
              and(eq(tickets.assignedTo, user.id), ne(tickets.status, "completed"), ne(tickets.status, "cancelled")),
            )
  ...
  private async updateAnalytics(data: {
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        await db.update(ticketAnalytics).set(updates).where(eq(ticketAnalytics.id, existing[0].id))
      } else {
  ...
  async getUserWorkload(userId: number): Promise<any> {
    try {
      const activeTickets = await db
        .select({ count: sql<number>`count(*)::int` })
        .from(tickets)
        .where(and(eq(tickets.assignedTo, userId), ne(tickets.status, "completed"), ne(tickets.status, "cancelled")))
  ...
  private async getExpertiseScore(userId: number, category: string): Promise<number> {
    try {
      ...
      const completedInCategory = await db
        .select({ count: sql<number>`count(*)::int` })
        .from(tickets)
        .where(and(eq(tickets.assignedTo, userId), eq(tickets.status, "completed"), eq(tickets.category, category)))
      ...
      const satisfaction = await db
        ...
        .from(tickets)
        .where(
          and(eq(tickets.assignedTo, userId), eq(tickets.status, "completed"), ne(tickets.customerSatisfaction, null)),
        )
  ...
  async getUserPerformanceMetrics(userId: number, dateRange?: { from: Date; to: Date }) {
    try {
      ...
      if (dateRange) {
        conditions.push(and(gte(tickets.createdAt, dateRange.from), lte(tickets.createdAt, dateRange.to)))
      }
      ...
      const staffWithWorkloads = await Promise.all(
        relevantStaff.map(async (staff) => {
          const activeTickets = await db
            ...
            .from(tickets)
            .where(
              and(eq(tickets.assignedTo, staff.id), ne(tickets.status, "completed"), ne(tickets.status, "cancelled")),
            )
